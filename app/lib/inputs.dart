...
class Inputs {
  ...
  factory Inputs() {
    final paths = Paths(
        packageRelativePath: packagePathInput.value ?? '',
        optionsRelativePath: customAnalysisOptions.value);
    ...
    if (paths.canonicalPathToAnalysisOptions != null &&
        !File(paths.canonicalPathToAnalysisOptions).existsSync()) {
      throw ArgumentError.value(
          paths.canonicalPathToAnalysisOptions,
          customAnalysisOptions.name,
          'The analysis options file has not been found');
    }
